...
class MessageRelay extends BaseComponent {
    ...
    initialize() {
        ...
        } else if (this.config.messageRelay && this.config.messageRelay.patterns) {
            // Auto-generate rules from messageRelay config
            this.setupRelayFromConfig();
        }

        this.logger.info(`Message relay initialized with ${this.relayRules.size} rule(s)`);
    }
    ...
    setupRelayFromConfig() {
        ...
        Object.entries(relayConfig.patterns).forEach(([category, pattern]) => {
            ...
            this.logger.debug(`Added relay rule for category: ${category} with prefix: ${prefix}`);
        });
    }
    ...
    addRelayRule(rule) {
        ...
        this.relayRules.set(ruleId, {
            ...
            transform: rule.transform || (msg => ({ topic: rule.targetTopic, payload: msg }))
        });
    ...
    async relayMessage(message, rule, sourceTopic) {
        try {
            ...
            this.relayCount++;
            this.logger.debug(`Relayed message from ${sourceTopic} to ${transformed.topic}`);
