...
// Test case for CleanRfidTamperAlarm message
const testCleanRfidTamperAlarmMessage = () => {
  // Example from requirements
  const topic = "V5008Upload/2437871205/OpeAck";
  const rawHex = "AA914EF665A1E20101024F015C55";
  const message = Buffer.from(rawHex, 'hex');
  
  console.log('Testing CleanRfidTamperAlarm message parsing...');
  ...
  if (result && result.msgType === 'CleanRfidTamperAlarm') {
    console.log('\n✅ Message type correctly identified as CleanRfidTamperAlarm');
    ...
    if (result.meta.msgId === 1325489237) {
    ...
    console.log('❌ Failed to identify message as CleanRfidTamperAlarm');
  }
};

// Run the test
testCleanRfidTamperAlarmMessage();